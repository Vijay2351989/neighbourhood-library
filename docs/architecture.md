...
|   gRPC-Web    |  HTTP |   gRPC-Web    |  HTTP/2|  LibraryServicer   |  TCP  |  library DB |
|   client      |       |   ↔ gRPC      |  gRPC  |  + 12 RPCs          |       |             |
|               |       |   filter      |  native|                     |       |             |
+---------------+       +---------------+       +---------------------+       +-------------+
                                                          ^
                                                          |
...
| Generated types | TypeScript message classes (`*_pb.ts`) and service descriptor (`*_connect.ts`), from `buf generate ../proto` | [Phase 3](phases/phase-3-proto-codegen.md) |
...
| Entry point | `backend/src/library/main.py` — registers `LibraryServicer`, `Health`, `ServerReflection` on one server | same |
...
| Generated code | `library_pb2.py` (messages) + `library_pb2_grpc.py` (stub + servicer base) — gitignored, regenerated by `scripts/gen_proto.sh` | [Phase 3](phases/phase-3-proto-codegen.md) |
...
The `.proto` is the single source of truth for the API surface. Both backend and frontend codegen from it: the backend gets typed `LibraryServiceServicer` to implement, the frontend gets a typed Connect-Web client. No drift between client and server, no DTOs to maintain, no OpenAPI to regenerate.
...
12 RPCs in one service, `library.v1.LibraryService`:

| Group | Methods | Purpose |
|---|---|---|
| **Books** | `CreateBook`, `UpdateBook`, `GetBook`, `ListBooks` | Catalog management. `CreateBook` takes `number_of_copies` and creates the matching `book_copies` rows transactionally. |
| **Members** | `CreateMember`, `UpdateMember`, `GetMember`, `ListMembers` | Patron records. Email is unique on `lower(email)` to prevent case-variant duplicates. |
| **Loans** | `BorrowBook`, `ReturnBook`, `ListLoans`, `GetMemberLoans` | Lending. `BorrowBook` picks any AVAILABLE copy via `FOR UPDATE SKIP LOCKED`, creates the loan, flips the copy status. `ReturnBook` does the reverse. |
...
Full proto definition (every message, every method, every field number) lives at `proto/library/v1/library.proto`. Detailed prose explanation: [`design/02-api-contract.md`](design/02-api-contract.md).
...
├── proto/library/v1/library.proto  # single source of truth — shared by both codegens
